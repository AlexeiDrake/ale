...
function! ale#code_actions#ApplyChanges(filename, line, column, end_line, end_column, text) abort
    let l:insertions = split(a:text, '\n')

    " The buffer is used to determine the fileformat, if available.
    let l:buffer = bufnr(a:filename)
    let l:lines = readfile(a:filename)

    let l:middle = [l:lines[a:line - 1][: a:column - 2] . l:insertions[0]]
    call extend(l:middle, l:insertions[1:])
    let l:middle[-1] .= l:lines[a:end_line - 1][a:end_column :]

    let l:lines = l:lines[: a:line - 2]
    \   + l:middle
    \   + l:lines[a:end_line :]
